    ...
    public void initialize(Map<String, URI> fmus, CoSimStepSizeCalculator stepSizeCalculator, Double endTime, List<ModelParameter> parameters,
        ...
        if (fmus.size() == 1) {
            ...
            if (outputs.entrySet().size() == 1) {
                ...
                Map.Entry<ModelConnection.ModelInstance, Set<ModelDescription.ScalarVariable>> singleFmuInstanceOutputs = outputs.entrySet()
                        .iterator().next();
                List<ModelDescription.ScalarVariable> singleFmuCorrelatedOutputs = singleFmuInstanceOutputs.getValue().stream().map(sv -> {
                    Optional<ModelDescription.ScalarVariable> correlatedScalars = modelDescScalars.stream().filter(svMd -> svMd.name.equals(sv.name))
                            .findFirst();
                ...
                ModelConnection.ModelInstance environmentFMUInstance = new ModelConnection.ModelInstance(environmentFMU.key,
                        environmentFMU.instanceName);
                //connections for: environment FMU outputs -> single FMU inputs
                List<ModelConnection> envFmuOutputsToSingleFmuInputs = createEnvConnection(environmentFMU.getOutputs(), environmentFMUInstance,
                        singleFmuInstanceOutputs.getKey());
