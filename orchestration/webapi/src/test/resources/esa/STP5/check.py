...
with tempfile.TemporaryDirectory() as directory:
    ...
    for i in range(0, 1):
        ...
        for ni in range(0, 1):
            ...
            print("Simulate 2")
            if sim1.simulate(json.dumps(json.load(open("2-simulateFor.json", encoding='utf8')))).status != 200:
                print("Simulate simulator 1 failed")
                failed = True
                break

            print("Simulate 3")
            if sim1.simulate(json.dumps(json.load(open("3-simulateFor.json", encoding='utf8')))).status != 200:
                print("Simulate simulator 1 failed")
                failed = True
                break
