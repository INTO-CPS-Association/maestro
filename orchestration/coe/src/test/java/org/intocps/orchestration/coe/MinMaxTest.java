...
public class MinMaxTest extends BasicTest {
    ...
    public static class MyAppender extends AbstractAppender {
        ...
        private final List<String> eventsList = new Vector<>();
        ...
        public void append(LogEvent event) {
            if (event != null && event.getLevel() == Level.WARN || event.getLevel() == Level.ERROR || event.getLevel() == Level.FATAL) {
                eventsList.add(event.getMessage().getFormattedMessage());
                //			System.out.println("############ "+this.getName()+" "+event.getMessage().getFormattedMessage());}
    ...
    public void setup() {
        ...
        //System.out.println("################# Changing appender: "+appender.getName());
        appender.start();
        //		LoggerContext lc = (LoggerContext) LogManager.getContext(false);
        ...
        //		LoggerConfig loggerConfig = LoggerConfig.newBuilder().withLoggerName(appender.getName()).withLevel(Level.ALL).withConfig(config)
        //				.build();
        //		loggerConfig.addAppender(appender, Level.ALL, null);

        // Add the LoggerConfig to the configuration
        //		config.addLogger(appender.getName(), loggerConfig);
        //		config.getLoggerConfig("org.intocps.orchestration.coe.scala.CoeObject").setLevel(Level.ALL);
        //		config.getLoggerConfig("org.intocps.orchestration.coe.scala.CoeObject").addAppender(appender,Level.ALL,null);
        config.getRootLogger().addAppender(appender, Level.ALL, null);

        context.updateLoggers();

        //"org.intocps.orchestration.coe.scala.CoeObject
        //
    ...
    boolean checkLogFor(String content) {
        for (String message : appender.eventsList) {
    ...
    void showAppenderCheckList() {
        //		System.out.println("##### Running check now: "+ this.appender.getName());
        //		this.appender.eventsList.forEach(event->System.out.println(appender.getName()+" "+event));

    }

    @Test
    public void testIntBoundOk() throws IOException, NanoHTTPD.ResponseException {
        ...
        showAppenderCheckList();
        Assert.assertFalse("Did not expect any out of bounds warning", checkLogFor("is out of bounds"));
    }

    @Test
    public void testIntBoundMin() throws IOException, NanoHTTPD.ResponseException {
    ...
    @Test
    public void testIntBoundMax() throws IOException, NanoHTTPD.ResponseException {
    ...
    @Test
    public void testRealBoundMin() throws IOException, NanoHTTPD.ResponseException {
    ...
    @Test
    public void testRealBoundMax() throws IOException, NanoHTTPD.ResponseException {
        ...
        FmuFactory.customFactory = new IFmuFactory() {
            ...
            public IFmu instantiate(File sessionRoot, URI uri) throws Exception {
                ...
                if (uri.toASCIIString().contains("watertank-c")) {
                    ...
                    when(comp.getReal(new long[]{2})).thenReturn(new FmuResult<>(Fmi2Status.OK, new double[]{realValue}));

                    when(comp.getInteger(new long[]{6})).thenReturn(new FmuResult<>(Fmi2Status.OK, new int[]{integerValue}));

                } else {
                    modelDescriptionPath = "src/test/resources/minMaxTest/watertankcontroller-c/modelDescription.xml";
                    when(comp.getBooleans(new long[]{4})).thenReturn(new FmuResult<>(Fmi2Status.OK, new boolean[]{true}));
