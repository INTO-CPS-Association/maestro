...
object CoeSimulator {
  ...
  /**
   * Prepare the simulation by setting up the experiment and notifying about the new initial state
   *
   * @param instances
   * @param outputs
   * @param inputs
  ...
  def prepareSimulation(instances: Map[ModelConnection.ModelInstance, FmiSimulationInstanceScalaWrapper],
    ...
    (initialized, initialStates)
  }

  /**
   * Utility method for setting up the simulation experiment and initializing all FMUs
   *
   * @param instances
   * @param outputs
   * @param inputs
   ...
   * @return
   */
  def setupExperimentAndInitializeFmus(instances: Map[ModelConnection.ModelInstance, FmiSimulationInstanceScalaWrapper],
                                       outputs: Outputs,
                                       inputs: Inputs, startTime: Double, endTime: Double, logLevels: Map[ModelInstance, java.util.List[String]], coe: Coe): Boolean = {
    if (logLevels != null) {

      setLogLevels(instances, logLevels)
    }

    fmiCallAll("Setup Experiment", instances, (instance: ModelInstance, i: IFmiComponent) => {
  ...
  def getRollbackStates(instances: Map[ModelInstance, FmiSimulationInstanceScalaWrapper]): Map[ModelInstance, IFmiComponentState] = {
    instances.filter(mi => mi._2.config.asInstanceOf[FmiInstanceConfigScalaWrapper].canGetSetState).map { mi =>
  ...
  def updateDerivatives(coe: Coe, state: GlobalState, stepsize: Double): GlobalState = {
    ...
    val readDerivatives = state.instanceStates.map { case (mi, instanceState) => mi -> {
    ...
    new GlobalState(state.instanceStates.map { case (mi, state) => mi -> {
  ...
  def doIntermediateStabalizationSteps(instances: Map[ModelInstance, FmiSimulationInstanceScalaWrapper],
    ...
    val instancesToStabalize = instances.filter { case (mi, d) => d.config.modelDescription.getCanGetAndSetFmustate }

    var state = initState
    var remainingStabalizationSteps = 5
    while (remainingStabalizationSteps > 0) {
      //rollback instances
      instanceStateMap.filterKeys(instancesToStabalize.keySet).foreach { case (mi, s) => instancesToStabalize.get(mi).get.instance.setState(s) }
      ...
      val f: Iterable[Boolean] = state.instanceStates.map { case (mi, s) => s.state.filter { case (sv, _) => sv.`type`.`type` == Types.Real }.map { case (sv, v) => {
  ...
  def doStep(instances: Map[ModelConnection.ModelInstance, FmiSimulationInstanceScalaWrapper],
    ...
    val doStepStatus = {
      ...
      if (paralleliseDoStep || coe.getConfiguration.parallelSimulation) {
        instances.par.map { case (mi, instance) => mi -> {
    ...
    //error handling
    if (!doStepStatus.forall { x => x._2 == Fmi2Status.OK }) {
      ...
      if (ex == null) {
        ...
        if (ex == null) {
          ...
          if (ex == null) {
            ...
            maxStepSize.foreach { x => logger.trace("possible rollback point: {}", x) }
  ...
  def setAllInputs(instances: Map[ModelInstance, FmiSimulationInstanceScalaWrapper], resolvedInputs: Map[ModelInstance, InputState], coe: Coe) = {
    ...
    val setInputFunc = (mi: ModelInstance, state: InputState) => {
      ...
      val types = state.inputs.keySet.map { sv => sv.`type`.`type` }

      types.foreach { fmiType =>
        Util.setRaw(comp, fmiType, {
          val valueRefTypeMap = state.inputs.filter { case (sv, _) => sv.`type`.`type` == fmiType }.map { case (sv, v) =>
            sv.valueReference -> v
          }
      ...
      if (instances(mi).config.modelDescription.getCanInterpolateInputs()) {
        ...
        val inPutDerivatives: Seq[(Long, Int, StepSize)] = state.derivatives.flatMap { case (sv, ders) => ders.map { case (order, value) => (sv.valueReference, order, value) }
        }.toSeq

        if (!inPutDerivatives.isEmpty) {

          val valueRefs: Array[Long] = inPutDerivatives.map { case (valueReference, _, _) => valueReference.toLong }.toArray
          val order: Array[Int] = inPutDerivatives.map { case (_, order, _) => order }.toArray
          val values: Array[Double] = inPutDerivatives.map { case (_, _, value) => value }.toArray
    ...
    if (paralleliseSetInputs || coe.getConfiguration.parallelSimulation) {
      resolvedInputs.par.foreach { case (mi, state) =>
        setInputFunc(mi, state)
      }
    } else {
      resolvedInputs.foreach { case (mi, state) =>
        setInputFunc(mi, state)
      }
    }
  }
  ...
  def resolveInputs(instances: Map[ModelInstance, FmiSimulationInstanceScalaWrapper], inputs: Inputs, state: GlobalState, coe: Coe): Map[ModelInstance, InputState] = {
    ...
    def extractInputState(sourceMi: ModelInstance, config: FmiInstanceConfig, input: Map[ScalarVariable, (ModelInstance, ScalarVariable)], globalState: GlobalState): InputState = {
      val instanceInputs = input.map { case (sv, (targetMi, targetSv)) => {
        ...
        val dersv: Map[Int, StepSize] = if (config.modelDescription.getCanInterpolateInputs) {
          ...
          if (derivatives != null && derivatives.containsKey(targetSv)) {
            ...
            globalState.instanceStates(targetMi).derivatives(targetSv).filter { case (order, _) => order <= maxOrder }
          } else {
            null
          }
        } else {
      ...
      val derivativeInputs: Map[ScalarVariable, Map[Int, StepSize]] = instanceInputs.filter { case (_, (_, derivatives)) => derivatives != null }.map { case (sv, (_, derivatives)) => sv -> derivatives }

      new InputState(instanceInputs.map { case (sv, (value, _)) => sv -> value }, derivativeInputs)
    }


    if (paralleliseResolveInputs || coe.getConfiguration.parallelSimulation) {
      inputs.par.map { case (mi, input) => mi -> extractInputState(mi, instances(mi).config, input, state) }.seq
    }

    else {
      inputs.map { case (mi, input) => mi -> extractInputState(mi, instances(mi).config, input, state) }
    }
  }
  ...
  def getGlobalState(outputs: Outputs, instances: Map[ModelInstance, FmiSimulationInstanceScalaWrapper], curTime: Double, stepsize: Double, coe: Coe): Either[GlobalState, Exception] = {
    ...
    val state1: GlobalStateMap = if (paralleliseObtainState || coe.getConfiguration.parallelSimulation) {
      outputs.keys.par.map { out => getOut(out)
      }.seq.toMap
    } else {
      outputs.keys.map { out => getOut(out)
      }.seq.toMap
    }
