...
public class SingleSimMain
{
			...
			extends SimulationExecutionUtil
	{
		...
		@Override protected void handleInitializationError(
				String initializeResponseData)
		{
			writeFailedStatus(initializeResponseData);
		}

		@Override protected void handleSimulateError(
				String simulateResponseData, NanoHTTPD.Response response)
		{
			writeFailedStatus(simulateResponseData);
		}

		@Override protected void handleDestroyError(NanoHTTPD.Response response)
		{
			try
			{
...
//			constraints.put("db-" + sv.getName(), constraint);
		...
		try
		{
			...
			new SimulationExecutionUtilStatusWriter(cmd.hasOption(verboseOpt.getOpt())).run("memoryConfig", config, startTime, endTime, outputFile);
		} catch (Exception e)
		{
			writeFailedStatus(
					e.getMessage() + "\n\n" + ExceptionUtils.getStackTrace(e));
