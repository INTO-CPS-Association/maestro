...
public class RequestProcessors {
    ...
    public static Map<ModelConnection.ModelInstance, List<String>> buildLogLevelsMap(Map<String, List<String>> logLevels) throws Exception {
        Map<ModelConnection.ModelInstance, List<String>> logLevelsParsed = new HashMap<>();
        for (Map.Entry<String, List<String>> entry : logLevels.entrySet()) {
            logLevelsParsed.put(ModelConnection.ModelInstance.parse(entry.getKey()), entry.getValue());
        }
        return logLevelsParsed;
    }
    ...
    public NanoHTTPD.Response processFileUpload(String sessionId,
                ...
                if (startBoundary.indexOf("--" + boundary) != -1) {
                    ...
                    do {
                        ...
                        if (partHeaders.contains("filename=")) {
                            // Extract filename
                            String[] filelist = partHeaders.split("filename=\"")[1].split("\"")[0].split(
                                    "\\"
                                            + System.getProperty("file.separator")); // Necessary with both?
                            filename = filelist[filelist.length - 1];
                            // Process the binary data in the part
                            String filePath =
                                    sessionController.getSessionRootDir(sessionId)
                                            + "/" + filename;
    ...
    public NanoHTTPD.Response processInitialize(
        ...
        try {
                    ...
                    if (size instanceof List) {
                        for (Object number : (List<Object>) objSize) {
                            ...
                            if (number instanceof Double) {
                                ...
                                return ProcessingUtils.newFixedLengthPlainResponse(NanoHTTPD.Response.Status.BAD_REQUEST,
                                        "size must be a 2-dimensional array of doubles: [minsize,maxsize]. One of the sizes "
                                                + number
                                                + " was not a double.");
                            }
                        }
            ...
            try {
                ...
                logs = coe.initialize(st.getFmuFiles(), buildConnections(st.connections), buildParameters(st.parameters), stepSizeCalculator, new LogVariablesContainer(buildVariableMap(st.livestream), buildVariableMap(st.logVariables)));
            ...
            return NanoWSDImpl.newFixedLengthResponse(NanoHTTPD.Response.Status.OK, Response.MIME_JSON, mapper.writeValueAsString(new ArrayList<>(Arrays.asList(ret))));
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private NanoHTTPD.Response simulate(String sessionId, String data,
        ...
        if (!async) {
            try {
                ...
                return NanoHTTPD.newFixedLengthResponse(NanoHTTPD.Response.Status.OK, Response.MIME_JSON, mapper.writeValueAsString(sessionController.getStatus(sessionId)));
            } catch (Exception e) {
    ...
    public NanoHTTPD.Response processSimulate(String sessionId, String data,
                                              boolean async) throws IOException {
        return simulate(sessionId, data, async);
    }

    public NanoHTTPD.Response processResult(String sessionId, boolean zip)
            throws IOException {
        Coe coe = sessionController.getCoe(sessionId);
    ...
    public NanoHTTPD.Response processDestroy(String sessionId)
            throws IOException {
