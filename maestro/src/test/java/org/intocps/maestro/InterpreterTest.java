...
public class InterpreterTest extends BaseApiTestAssertion {
    ...
    public static Collection<Object> expectedExceptionData() {
        return Arrays.stream(
                Objects.requireNonNull(Paths.get("src", "test", "resources", "InterpreterTest", "expectedException").toFile().listFiles())).map(f -> {
            try {
                return new Object[]{f.getName(), FileUtils.readFileToString(f, StandardCharsets.UTF_8)};
            } catch (IOException e) {
                e.printStackTrace();
