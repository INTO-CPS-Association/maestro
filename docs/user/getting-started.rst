...
The next example presents the new specification generated after the expansion of line 63 containing :code:`FixedStep.expand fixedStep(...)`. Thus the :code:`FixedStep.expand fixedStep(...)` has been replaced with the MaBL generated by the corresponding FixedStep plugin. As it can be seen on line 61 and 62, :code:`@Config(...)` and :code:`expand initialize(...)` have not been expanded yet.
