...
The next example presents the intermediate expansion `spec00002.mabl` generated after the expansion of line 63 in `spec00000.mabl` containing :code:`FixedStep.expand fixedStep(...)`.
Thus the :code:`FixedStep.expand fixedStep(...)` has been replaced with the MaBL generated by the corresponding Fixedstep plugin as seen on line 63 to 202.
As it can be seen on line 61 and 62, :code:`@Config(...)` and :code:`expand initialize(...)` have not been expanded yet.
