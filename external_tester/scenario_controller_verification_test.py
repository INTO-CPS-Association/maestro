...
def verifyAlgorithmTest(SCR_path, jarPath):
    ...
    cmd = "java -jar {0} verifyAlgorithm {1} -output {2}".format(jarPath, masterModelPath, temporary)
    func = lambda: True
...
def visualizeTracesTest(SCR_path, jarPath):
    ...
    cmd = "java -jar {0} visualizeTraces {1} -output {2}".format(jarPath, masterModelPath, temporary)
    func = lambda: True
    testutils.testCliCommandWithFunc(cmd, func)
