...
public class VdmSvChecker
{
	...
	{
		List<ModelDescription.ScalarVariable> SV_list = new ArrayList<ModelDescription.ScalarVariable>();
	...
	private static ModelDescription.Initial convertInitialSV(Object initial)
	{
		...
		if (initial instanceof approxQuote) return ModelDescription.Initial.Approx;
		else if (initial instanceof calculatedQuote) return ModelDescription.Initial.Calculated;
		else if (initial instanceof exactQuote) return ModelDescription.Initial.Exact;
		else return null;
	}
	...
	private static ModelDescription.Variability convertVariabilitySV(Object variability)
	{
		...
		if (variability instanceof constantQuote) return ModelDescription.Variability.Constant;
		else if (variability instanceof continuousQuote) return ModelDescription.Variability.Continuous;
		else if (variability instanceof discreteQuote) return ModelDescription.Variability.Discrete;
		else if (variability instanceof fixedQuote) return ModelDescription.Variability.Fixed;
		else if (variability instanceof tunableQuote) return ModelDescription.Variability.Tunable;
		else return null;
	}
	...
	private static ModelDescription.Causality convertCausalitySV(Object causality)
	{
		...
		if (causality instanceof calculatedParameterQuote) return ModelDescription.Causality.CalculatedParameter;
		else if (causality instanceof independentQuote) return ModelDescription.Causality.Independent;
		else if (causality instanceof inputQuote) return ModelDescription.Causality.Input;
		else if (causality instanceof localQuote) return ModelDescription.Causality.Local;
		else if (causality instanceof outputQuote) return ModelDescription.Causality.Output;
		else if (causality instanceof parameterQuote) return ModelDescription.Causality.Parameter;
		else return null;
	}
}
