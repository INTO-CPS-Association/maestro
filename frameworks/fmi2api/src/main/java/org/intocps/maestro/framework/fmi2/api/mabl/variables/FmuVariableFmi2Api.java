...
public class FmuVariableFmi2Api extends VariableFmi2Api<Fmi2Builder.NamedVariable<PStm>> implements Fmi2Builder.Fmu2Variable<PStm> {
    ...
    public void freeInstance(Fmi2Builder.Scope<PStm> scope, Fmi2Builder.Fmi2ComponentVariable<PStm> comp) {
        if (comp instanceof ComponentVariableFmi2Api) {
            scope.add(newIf(newNotEqual(((ComponentVariableFmi2Api) comp).getReferenceExp().clone(), newNullExp()),
                    newABlockStm(MableAstFactory
                    .newExpressionStm(call(getReferenceExp().clone(), "freeInstance", ((ComponentVariableFmi2Api) comp).getReferenceExp().clone())),
                            newAAssignmentStm(((ComponentVariableFmi2Api) comp).getDesignator(), newNullExp())), null));
        } else {
