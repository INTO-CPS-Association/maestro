...
public class Fmi3InterpreterAuto {
    final static Logger logger = LoggerFactory.getLogger(Interpreter.class);
    private final File workingDirectory;

    public Fmi3InterpreterAuto(File workingDirectory) {
    ...
    public static Map<String, Value> createFmuMembers(File workingDirectory, String guid, IFmi3Fmu fmu) {
        ...
        functions.put("instantiateCoSimulation", new FunctionValue.ExternalFunctionValue(fargs -> {
            ...
            long[] requiredIntermediateVariables =
                    getArrayValue(fargs.get(5), Optional.empty(), NumericValue.class).stream().mapToLong(NumericValue::longValue).toArray();
            ...
            try {
                ...
                ILogMessageCallback logCallback = (instanceName, status, category, message) -> {
                    logger.info("NATIVE: instance: '{}', status: '{}', category: '{}', message: {}", instanceName, status, category, message);
                    {
                        ...
                        Log4jLogEvent.Builder builder =
                                Log4jLogEvent.newBuilder().setMessage(new ParameterizedMessage(formatter, category, status, instanceName, message));
                ...
                IIntermediateUpdateCallback intermediateUpdateCallback =
...
int activateModelPartition(uint clockReference, real activationTime);
        ...
        */
        ...
        componentMembers.put("doStep", new FunctionValue.ExternalFunctionValue(fcargs -> {
            ...
            try {
                FmuResult<IFmi3Instance.DoStepResult> res =
                        instance.doStep(currentCommunicationPoint, communicationStepSize, noSetFMUStatePriorToCurrentPoint);
        ...
        return new Fmu3InstanceValue(componentMembers, instance, fmuLogOutputStream);
    }
        ...
        for (Map.Entry<String, TP> method : methodTypeMapping.entrySet()) {
            if (method.getKey().startsWith("get")) {
                ...
                componentMembers.put(method.getKey(), new FunctionValue.ExternalFunctionValue(fcargs -> {
                    ...
                    long[] scalarValueIndices =
                            getArrayValue(scalarValue, Optional.empty(), NumericValue.class).stream().mapToLong(NumericValue::longValue).toArray();
