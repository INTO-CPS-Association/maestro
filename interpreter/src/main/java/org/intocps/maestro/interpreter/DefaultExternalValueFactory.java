...
public class DefaultExternalValueFactory implements IExternalValueFactory {
    ...
    public static class Fmi3LifecycleHandler extends BaseLifecycleHandler {
        final private File workingDirectory;

        public Fmi3LifecycleHandler(File workingDirectory) {
            this.workingDirectory = workingDirectory;
        }
        ...
        public Either<Exception, Value> instantiate(List<Value> args) {
            ...
            return Either.right(new Fmi3Interpreter(workingDirectory).createFmiValue(path, guid));
        }
    }
