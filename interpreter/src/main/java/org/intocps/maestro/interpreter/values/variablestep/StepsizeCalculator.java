...
public class StepsizeCalculator
{
	...
	private CurrentSolutionPoint currentSolutionPoint = new CurrentSolutionPoint();
	private List<ConstraintHandler> handler = new Vector<ConstraintHandler>();
	...
	private StepsizeInterval stepsizeInterval;
	private FmuMaxStepSizeHandler fmuMaxStepSizeHandler = null;

	public StepsizeCalculator(final Set<InitializationMsgJson.Constraint> constraints,
			final StepsizeInterval stepsizeInterval,
			final Double initialStepsize,
			final Map<ModelConnection.ModelInstance, FmiSimulationInstance> instances)
					throws AbortSimulationException
	{
		...
		for (InitializationMsgJson.Constraint constraint : constraints)		{
			try{
				ConstraintHandler h = ConstraintHandlerFactory.getHandler(currentSolutionPoint, constraint, stepsizeInterval, STRONG_RELAXATION_FACTOR,getTypeMap(instances),logger);
				if (h == null)
				{
				...
				throw new AbortSimulationException("The simulation has been aborted because the constraint '" + constraint.getId() + "' could not be instantiated. Details: "+e.getMessage(),e);
			}
		}
	}
	
	private Map<Variable,Types> getTypeMap(final Map<ModelConnection.ModelInstance, FmiSimulationInstance> instances){
		Map<Variable,Types> map = new HashMap<Variable,Types>();			
	...
	public Double getStepsize(final Double currentTime,
	...
	public StepValidationResult validateStep(final Double nextTime,
			...
			final Boolean supportsRollback)
	{
		...
		{
			reducedStepsize = Math.max(stepsize
					* INVALID_STEP_TIGHTENING_FACTOR, stepsizeInterval.getMinimalStepsize());
	...
	private void produceLogOutput(final Double currentTime,
			final Set<ConstraintHandler> limitingConstraints)
	{
		if (limitingConstraints.size() > 1
				&& areAllConstraintsRelaxingStrongly(limitingConstraints))
		{
	...
	private Boolean areAllConstraintsRelaxingStrongly(
			final Set<ConstraintHandler> limitingConstraints)
	{
	...
	private Boolean containsOnlyDiscreteConstraints(
			Set<ConstraintHandler> handlers)
	{
	...
	private Set<ConstraintHandler> findLimitingConstraints(
			Map<ConstraintHandler, Double> maxStepsizes)
	{
		Set<ConstraintHandler> limitingConstraints = new HashSet<ConstraintHandler>();
	...
	private void logLimitingConstraintsDecisions(final Double currentTime,
			Set<ConstraintHandler> limitingConstraints)
	{
		String msg = "Time " + currentTime + ", stepsize " + stepsize;
		for (ConstraintHandler h : limitingConstraints)
		{
			msg += ", limited by constraint \"" + h.getId()
					+ "\" with decision to " + h.getDecision();
		}
		logger.debug(msg);
	}
	...
	private void logFmuRequiredConstraint(final Double currentTime,
	...
	{
		final Map<ConstraintHandler, Double> stepsizes = new HashMap<ConstraintHandler, Double>();
		for (ConstraintHandler h : handler)
		{
